...
def operator(d=1, tail_std=0.0, trunk_activation='tanh'):
    return OperatorFilter(data_size=d, latent_dim=d, gru_hidden=8, ctx_dim=8, p=8,
                          branch_hidden=8, trunk_hidden=8, trunk_layers=1,
                          tail_std=tail_std, trunk_activation=trunk_activation, key=jax.random.PRNGKey(4))
