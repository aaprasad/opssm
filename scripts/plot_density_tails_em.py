...
def main():
    ...
    if not settings['learn_dynamics']:
        parser.error('this plot requires a --learn-dynamics experiment')
    if len(runs) != len(settings['seeds'])*len(settings['tail_stds']):
    ...
    for idx, tail in enumerate(settings['tail_stds']):
        subset = [r for r in runs if r['tail_std'] == tail]
        label = 'Legacy' if tail == 0 else f'Gaussian tail (std={tail:g})'
        color = colors(idx)
        ...
        for run in subset:
            ...
            drift = eqx.tree_deserialise_leaves(out / name, skeleton)
            fields.append(np.asarray(drift.net(jnp.asarray(field_z[:, None])))[:, 0])
            summary.append(dict(seed=run['seed'], tail_std=tail,
    ...
    fig.suptitle('Learned drift and diffusion: Gaussian-tail ablation\n'
    ...
    for tail in settings['tail_stds']:
        rows = [r for r in summary if r['tail_std'] == tail]
        print(f'tail_std={tail:g}: ' + ', '.join(
